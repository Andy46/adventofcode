...
def findGuard(data):
    for x in range(0, len(data)):
        for y in range(0, len(data[0])):
            if data[x][y] == INITIAL_GUARD_TOKEN:
                return x,y
...
# Calculate
initialGuard = findGuard(initialData), 'n'

def calcLoopFunction(guardPos):
    obstacleCell = getNextCell(guardPos[0][0], guardPos[0][1], guardPos[1])
    if isCellInData(data, obstacleCell[0], obstacleCell[1]):
        newData = addObstacle(copy.deepcopy(initialData), obstacleCell)
        if PRINT_MAP:
            printMap(data)
            printMap(newData)
        # Test new map
...
with Pool(processes=MAX_THREADS) as pool:
    loopPositions = pool.map(calcLoopFunction, MainHistory)
loopPositions = set(loopPositions)
loopPositions.remove(None)
