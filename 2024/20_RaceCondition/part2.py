...
def countShortcuts(maze, shortlen):
    ...
    with click.progressbar(NO_XY_POSITIONS) as bar:
        for currentPosition in bar:
            if not isWall(maze, currentPosition):
                allNextPositions = [position for position in getAllPositionsM(currentPosition, shortlen) if isValidPosition(maze, position) and not isWall(maze,position)]
                ...
                for nextPosition in allNextPositions:
                    ...
                    if currentValue < nextValue:
                        ...
                        totalSaved.append(((nextValue-currentValue) - distance))
    return totalSaved
...
if __name__ == "__main__":
    ...
    # Output result
    # lowestScore = getMazeCell(filledMaze, START)
