...
int determineMinimumTokens(const Button& buttonA, const Button& buttonB, const std::vector<int>& prize) {
    ...
    for (const auto& combination : combinations) {
        int countA = combination.first;
        int countB = combination.second;
...
int main() {
    ...
    std::cout << "Calculating machines in parallel..." << std::endl;

    // Crear tareas paralelas para calcular los tokens
    int id = 0;
    int machineCount = initialData.size();
    std::vector<std::future<int>> futures;
    for (const auto& machine : initialData) {
        id++;
        futures.push_back(std::async(std::launch::async, [id, machineCount, &machine]() {
            std::cout << "Machine " << id << "/" << machineCount << "..." << std::endl;
            const auto& buttons = machine.buttons;
            const auto& prize = machine.prize;
            int min = determineMinimumTokens(buttons[0], buttons[1], prize);
            std::cout << "Machine " << id << "/" << machineCount << " finished" << std::endl;
            return min;
        }));
    }
