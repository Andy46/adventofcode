...
def calcDistance3D(boxA, boxB):
    ...
    dist3D = sum([abs((a-b)*(a-b)) for a,b in zip(boxA, boxB)])
    return dist3D
...
def makeConnections(distances : list[Distance], boxCount) -> Distance:
    circuits = []
    for dist in distances:
        ...
        if len(circuits) == 1 and len(circuits[0]) == boxCount:
            return dist
    return None

def calcXDistance(dist : Distance) -> int:
    return dist.boxA[0] * dist.boxB[0]
...
if __name__=="__main__":
    ...
    # printData(distances)
    lastConnection = makeConnections(distances, len(boxes))
    print(lastConnection)
    
    print(f"Value: {calcXDistance(lastConnection)}")  
